...
export function buildWorkoutFit({ session, notes, allExercises }) {
  ...
  const hrRecords = session?.hrRecords || [];

  const startTime = new Date(`${session.date}T12:00:00`);
  let elapsedSeconds = Math.max(60, Math.round(session.duration || 0));
  if (hrRecords.length > 0) {
    elapsedSeconds = Math.max(elapsedSeconds, hrRecords[hrRecords.length - 1].t);
  }
  ...
  // Heart rate stream imported from a watch FIT export, so Strava can render
  // an HR graph for manually-logged sessions.
  for (const rec of hrRecords) {
    encoder.writeMesg({
      mesgNum: Profile.MesgNum.RECORD,
      timestamp: new Date(startTime.getTime() + rec.t * 1000),
      heartRate: rec.hr,
    });
  }
  ...
  encoder.writeMesg({
    ...
    ...(session.calories > 0 ? { totalCalories: Math.round(session.calories) } : {}),
  });
