...
export function applyDecay(xp, lastTrainedDate) {
  ...
  return Math.max(0, xp - decayDays * DECAY_PER_DAY);
}
...
// Returns: { chest: { rawXp, effectiveXp, level, currentXp, xpNeeded, lastTrained, streak, multiplier }, ... }
// ---------------------------------------------------------------------------
export function computeMuscleStats(allNotes, strength, allExercises) {
  ...
  // Now compute total XP with streak multiplier applied per week
  const result = {};
  for (const mg of allMuscleIds) {
    // Sort weeks chronologically
    const sortedWeeks = Object.keys(weeklyXp[mg]).sort();
    // Build streak at each point in time to apply the right multiplier
    let runningStreak = 0;
    let prevWk = null;
    let rawXp = 0;
    ...
    for (const wk of sortedWeeks) {
      ...
      if (prevWk) {
        const prev = new Date(prevWk.replace(/W(\d+)/, (_, w) => {
          const jan4 = new Date(parseInt(prevWk), 0, 4);
          const start = new Date(jan4);
          start.setDate(jan4.getDate() - ((jan4.getDay() + 6) % 7));
          start.setDate(start.getDate() + (parseInt(w) - 1) * 7);
          return start.toISOString().slice(5, 10).replace("-", "/");
        }));
...
export function diffMuscleStats(before, after) {
  let totalXp = 0;
  const levelUps = [];
  for (const mg of Object.keys(after)) {
...
export function generateMonthlyChallenge(muscleStats, month, allExercises) {
  const allMuscleIds = Object.keys(muscleStats);
...
// ---------------------------------------------------------------------------
export function computeChallengeProgress(challenge, notes, strength) {
