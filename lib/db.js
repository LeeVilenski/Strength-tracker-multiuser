...
export async function getTopBestEfforts(nameKey, limit = 15) {
  const rows = await sql`
    SELECT be.activity_id, be.distance, be.moving_time, a.date, a.name AS activity_name
...
// Upload a structured Workout Log JSON payload (requires activity:write scope)
export async function uploadStrengthActivity(jsonPayload, externalId, sportType = "Workout") {
  const accessToken = await getValidAccessToken();
  if (!accessToken) throw new Error("Not authenticated");
  ...
  form.append("file", new Blob([JSON.stringify(jsonPayload)], { type: "application/json" }), "workout.json");
...
// Poll the status of a Strava upload
export async function getStravaUploadStatus(uploadId) {
  const accessToken = await getValidAccessToken();
  if (!accessToken) throw new Error("Not authenticated");
  const res = await fetch(`https://www.strava.com/api/v3/uploads/${uploadId}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) throw new Error(`Strava upload status fetch failed: ${res.status}`);
  return res.json();
}
