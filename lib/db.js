...
export async function getMonthlyChallenges() {
  const rows = await sql`SELECT * FROM monthly_challenges ORDER BY month_key DESC`;
  return rows.map(r => ({
...
export async function saveMonthlyChallenge(monthKey, challenge) {
  await sql`
    INSERT INTO monthly_challenges (month_key, challenge_json)
    VALUES (${monthKey}, ${JSON.stringify(challenge)})
    ON CONFLICT (month_key) DO NOTHING
  `;
}

export async function completeMonthlyChallenge(monthKey, bonusXp) {
  await sql`
    UPDATE monthly_challenges
    SET completed = TRUE, bonus_xp = ${bonusXp}, updated_at = NOW()
    WHERE month_key = ${monthKey} AND completed = FALSE
  `;
}
